...
function M.save_existing()
  for _, entry in ipairs(default_key_maps) do
  ...
  for _, custom in ipairs(custom_key_maps) do
    local custom_modes = wrap_in_table(custom[1])
    local custom_keys = wrap_in_table(custom[2])
    if custom[3] then
      for _, m in ipairs(custom_modes) do
        for _, k in ipairs(custom_keys) do
          save_existing_key_map(m, k)
        end
      end
    end
  end
...
local function custom_function(func)
  ...
  if common.is_mode("v") then
    virtual_cursors.visual_mode(function(vc) func(register, count) end)
  else
    virtual_cursors.edit_with_cursor(function(vc) func(register, count) end)
  end
end
...
local function custom_function_with_motion(func)
  ...
  if motion_cmd == nil then return end
  with_onemore(function() func(register, count, motion_cmd) end)
  if common.is_mode("v") then
    virtual_cursors.visual_mode(function(vc) func(register, count, motion_cmd) end)
  else
    virtual_cursors.edit_with_cursor(function(vc) func(register, count, motion_cmd) end)
  end
end
...
local function custom_function_with_char(func)
  ...
  if common.is_mode("v") then
    virtual_cursors.visual_mode(function(vc) func(register, count, char) end)
  else
    virtual_cursors.edit_with_cursor(function(vc) func(register, count, char) end)
  end
end
...
local function custom_function_with_motion_then_char(func)
  ...
  if common.is_mode("v") then
    virtual_cursors.visual_mode(function(vc) func(register, count, motion_cmd, char) end)
  else
    virtual_cursors.edit_with_cursor(function(vc) func(register, count, motion_cmd, char) end)
  end
end
...
function M.set_custom()
  for _, custom_key_map in ipairs(custom_key_maps) do
    ...
    if func then
      local wrapped = function() custom_function(func) end
      if #custom_key_map >= 4 then
        local opt = custom_key_map[4]
        if opt == "m" then
          wrapped = function() custom_function_with_motion(func) end
        elseif opt == "c" then
          wrapped = function() custom_function_with_char(func) end
        elseif opt == "mc" then
          wrapped = function() custom_function_with_motion_then_char(func) end
        end
      end
      for _, m in ipairs(custom_modes) do
        for _, k in ipairs(custom_keys) do
