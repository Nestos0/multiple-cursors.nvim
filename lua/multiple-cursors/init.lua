...
function M.setup(opts)
	...
	if opts.default_key_maps then
		for _, item in ipairs(opts.default_key_maps) do
			table.insert(common_key_maps, vim.inspect(item))
		end
	else
		for _, item in ipairs(default_key_maps) do
			table.insert(common_key_maps, vim.inspect(item))
		end
	end
	...
	-- Set up key maps
	key_maps.setup(common_key_maps, custom_key_maps)
